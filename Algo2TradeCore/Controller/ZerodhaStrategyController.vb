...
#Region "Common tasks for all strategies"
        ...
        Public Overrides Async Function SubscribeStrategyAsync(ByVal strategyToRun As Strategy) As Task
            ...
            End If
        End Function
        Public Overrides Async Function UnSubscribeStrategyAsync(strategyToRun As Strategy, instrumentsToUnsubscribe As List(Of IInstrument)) As Task
            logger.Debug("UnSubscribeStrategyAsync, strategyToRun:{0}", strategyToRun.ToString)
            _cts.Token.ThrowIfCancellationRequested()

            If strategyToRun IsNot Nothing Then
                Dim ret As Boolean = Await strategyToRun.RemoveTradableStrategyInstrumentsAsync(instrumentsToUnsubscribe).ConfigureAwait(False)

                'Now remove the instruments from unique instrument list
                If instrumentsToUnsubscribe IsNot Nothing AndAlso instrumentsToUnsubscribe.Count > 0 Then
                    For Each runningTradableInstrument In instrumentsToUnsubscribe
                        _cts.Token.ThrowIfCancellationRequested()
                        If _AllStrategies IsNot Nothing AndAlso _AllStrategies.Where(Function(x)
                                                                                         If x.TradableInstrumentsAsPerStrategy IsNot Nothing Then
                                                                                             Return x.TradableInstrumentsAsPerStrategy.Where(Function(y)
                                                                                                                                                 Return y.InstrumentIdentifier = runningTradableInstrument.InstrumentIdentifier
                            ...
                            Continue For
                        Else
                            _AllStrategyUniqueInstruments = _AllStrategyUniqueInstruments.Where(Function(z)
                                                                                                    Return z.InstrumentIdentifier <> runningTradableInstrument.InstrumentIdentifier
                                                                                                End Function)
                        End If
                    Next
                End If
                ...
                _cts.Token.ThrowIfCancellationRequested()
                'Now create instrument mapping
                Await RefreshInstrumentMappingTable().ConfigureAwait(False)
                ...
                'Now create a local collection inside the controller for the strategy instruments that need to be subscribed as a dictionary for easy picking during tick receive
                If _subscribedStrategyInstruments IsNot Nothing AndAlso instrumentsToUnsubscribe IsNot Nothing Then

                    Dim subscribedInstrumentTokens As List(Of String) = Nothing
                    For Each runningTradableStrategyInstrument In instrumentsToUnsubscribe
                        _cts.Token.ThrowIfCancellationRequested()
                        Dim instrumentKey As String = runningTradableStrategyInstrument.InstrumentIdentifier
                        Dim strategiesToBeSubscribedForThisInstrument As Concurrent.ConcurrentBag(Of StrategyInstrument) = Nothing
                        If _subscribedStrategyInstruments.ContainsKey(instrumentKey) Then
                            strategiesToBeSubscribedForThisInstrument = _subscribedStrategyInstruments(instrumentKey)
                        Else
